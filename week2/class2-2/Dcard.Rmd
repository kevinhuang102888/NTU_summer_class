...
7.套用function，將所有內文跟留言斷詞，並做次數累積
...
#cut the comment
txt2<-unlist(lapply(Dcard$comment[1:100],myFUN,'n'))
st2<-freq(txt2)
st2<-st2[order(st2$freq,decreasing = TRUE),]
```
...
```{r}
wordcloud(words=st1$char,freq=st1$freq,scale = c(2.5,0.1), # 給定文字尺寸的區間（向量）
...
```{r}
wordcloud(words=st2$char,freq=st2$freq,
          scale = c(2.5,0.1),random.order = F,
          ordered.colors = F,rot.per = F,
