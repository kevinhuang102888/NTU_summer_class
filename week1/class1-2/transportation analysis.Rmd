...
survey2$v2_1<-factor(survey2$v2_1,labels=c('新北市','宜蘭縣','桃園縣','新竹縣','苗栗縣',
          '原台中縣','彰化縣','南投縣','雲林縣','嘉義縣',
          '原台南縣','原高雄縣','屏東縣','台東縣','花蓮縣',
          '澎湖縣','基隆市','新竹市','台中市','嘉義市',
          '台南市','台北市','高雄市','金門縣','連江縣'))
...
certaindata<-function(data,expression){
  certain<-data%>%
    ...
    filter(expression&as.integer(newdata$transportation)>=14&as.integer(newdata$transportation)<=19)
  certain
}
...
total<-newdata%>%
    ...
    filter(expression1)
h1<-count(total,location)
h2<-count(youngadult,location)
rate<-data.frame(h1[1],1-h2[2]/h1[2])
names(rate)<-c('location','rate')
rate

levels(newdata$transportation)
